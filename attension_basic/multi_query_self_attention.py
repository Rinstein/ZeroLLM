...
class MQA(nn.Module):
    def __init__(self, input_size, head_num, embed_size):
        ...
        self.qkv_proj = nn.Linear(input_size, embed_size + self.head_dim * 2)
        self.out_proj = nn.Linear(embed_size, embed_size)

    def forward(self, x):
        qkv = self.qkv_proj(x)
        q, k, v = qkv.split([self.embed_size, self.head_dim, self.head_dim], dim=-1)
...
if __name__ == "__main__":
    ...
    net = MQA(input_size=input_size, head_num=head_num, embed_size=embed_size)
